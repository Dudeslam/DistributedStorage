...
def get_store_file_tasks(file_data, max_erasures):
    """
    ...
    for i in range(STORAGE_NODES_NUM):
        ...
        task = messages_pb2.file()
        task.filename = name
        task.type = "STORE_FRAGMENT_DATA_REQ"

        tasks.append(task)
        data.append(coefficients[:symbols] + bytearray(symbol))
...
def get_file_tasks(coded_fragments, max_erasures, file_size):
    ...
    for name in fragnames:
        task = messages_pb2.broadcast_request_fragment()
        task.filename = name
        task.type = 'FRAGMENT_DATA_REQ'
        tasks.append(task)
